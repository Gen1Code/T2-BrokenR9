...
var tick = (elapsedTime, multiplier) => {
    ...
    q1 = q1 + bonus * dt * getDQ1(dq1.level) * q2;
    q2 = q2 + bonus * dt * getDQ2(dq2.level) * (qTerms.level > 0 ? q3 : BigNumber.ONE);
    if (qTerms.level > 0) q3 = q3 + bonus * dt * getDQ3(dq3.level) * (qTerms.level > 1 ? q4 : BigNumber.ONE);
    if (qTerms.level > 1) q4 = q4 + bonus * dt * getDQ4(dq4.level);

    r1 = r1 + bonus * dt * getDR1(dr1.level) * r2;
    r2 = r2 + bonus * dt * getDR2(dr2.level) * (rTerms.level > 0 ? r3 : BigNumber.ONE);
    if (rTerms.level > 0) r3 = r3 + bonus * dt * getDR3(dr3.level) * (rTerms.level > 1 ? r4 : BigNumber.ONE);
    if (rTerms.level > 1) r4 = r4 + bonus * dt * getDR4(dr4.level);
